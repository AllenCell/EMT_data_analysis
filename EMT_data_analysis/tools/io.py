...
def load_image_analysis_extracted_features(load_from_aws: bool = True):
    ...
    if load_from_aws:
        print("Downloading features file (~6 GB) from AWS. This may take several minutes depending on your internet connection. Please wait...")
    df = pd.read_csv(path)
    return df
...
def load_inside_outside_classification(load_from_aws: bool = True):
    ...
    if not load_from_aws:
        ...
        path = metric_comp_results_dir / "Migration_timing_through_mesh_extracted_features.csv"
    df = pd.read_csv(path)
    return df

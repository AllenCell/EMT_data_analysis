...
def main(
    ...
    '''
        Generate figures for the inside-outside classification of nuclei
    ...
    pl.zoom_camera(0.8)
    for tp in tqdm(range(45)):
        pl.clear()

        # get the data for the timepoint
        time = tp
        tp1 = tp * 2
        df_tp = df[df['Time hr'] == time * 4]

        # add mesh to the scene
        pl.add_mesh(meshes[f'{tp1}'], color='#66b2b2', opacity=.5, show_edges=False, smooth_shading=True, specular=0.5, specular_power=15)

        # add nuclei centroids to the scene
        # Yellow = inside nuclei (contained within lumenoid)
        ...
        # Cyan = outside nuclei (migrated through basement membrane)
        ...
        pl.add_legend(
            labels=[
                ('Inside (pre-EMT)', 'yellow'),
                ('Outside (migrated)', 'cyan'),
                ('Collagen IV mesh', '#66b2b2')
            ],
            bcolor='white',
            face='circle',
            size=(0.2, 0.15)
        ...
        pl.add_title(f'Time: {time * 4} hours')

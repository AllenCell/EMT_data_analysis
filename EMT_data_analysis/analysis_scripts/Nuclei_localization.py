...
def nuclei_localization(
    ...
    if out_fn.exists():
        print(f"Skipping {data_id}: output already exists at {out_fn}")
        return
...
def _combine_results(output_directory:str):
    """
    Combine all individual per-movie localized nuclei CSVs into a single
    combined CSV matching the format of Migration_timing_through_mesh_extracted_features.csv.

    Columns: Data ID, Nucleus label, Inside, Time hr, X, Y, Z
    """
    out_dir = Path(output_directory)
    csv_files = sorted(out_dir.glob("*_localized_nuclei.csv"))
    ...
    # Rename and select columns to match target format
    combined = combined.rename(columns={"Label": "Nucleus label"})
    combined["Inside"] = combined["Inside"].astype(float)
    combined["Nucleus label"] = combined["Nucleus label"].astype(float)
    combined = combined[["Data ID", "Nucleus label", "Inside", "Time hr", "X", "Y", "Z"]]

    out_fn = out_dir / "Migration_timing_through_mesh_extracted_features.csv"
    combined.to_csv(out_fn, index=False)
    print(f"Combined CSV saved: {out_fn} ({len(combined)} rows from {len(csv_files)} movies)")
...
def run_nuclei_localization(
    ...
    # Combine all individual CSVs into a single output file
    _combine_results(output_directory)
